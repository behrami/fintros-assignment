class NewStoriesController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      format.html do
        if request.xhr?
            @new_stories_array.clear
            @new_stories_array = @new_stories_stored.clone
            @new_stories_stored.clear
      ...
      format.json {render json: @new_stories_stored}
    end
  ...
  def display_array_setup(first,last)
    ...
    (i..n).each do
      single_response = HTTParty.get("https://hacker-news.firebaseio.com/v0/item/#{@new_response[i]}.json")
  ...
  def stored_array_setup(first,last)
    ...
    (i..n).each do
      ...
      @new_stories_stored.push(single_story_hash)
